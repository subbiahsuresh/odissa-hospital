...
public class OdishaHospitalScraperToExcel {
    public static void main(String[] args) {
        String url = "http://example.com/odisha-hospitals"; // Replace with actual URL
        String excelFilePath = "odisha_hospitals.xlsx"; // Path to Excel file
        try {
            Document doc = Jsoup.connect(url).get();
            Elements hospitals = doc.select("div.hospital"); // Adjust CSS selector as per the HTML structure

            Workbook workbook = new XSSFWorkbook();
            Sheet sheet = workbook.createSheet("Odisha Hospitals");
            int rowNum = 0;
